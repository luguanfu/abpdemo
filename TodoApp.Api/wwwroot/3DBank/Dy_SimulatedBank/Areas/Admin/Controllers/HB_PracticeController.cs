...
{
    public class HB_PracticeController : BaseController
    {
        //
        // GET: /Admin/HB_Practice/
        ...
        public string GetList()
        {
            ...
            wheres += " and E_AddOperator=" + UserId + " and E_Type=2";
        ...
        public string Edit()
        {
            try
            {
                ...
                //校验考试名称是否存在
                string wheres = " and E_Type=" + E_Type + " and E_Name='" + E_Name + "' and EId!=" + EId;//试卷状态
        ...
        /// </summary>
        /// <returns></returns>
        public int IinkExercisePracticeChapters()
        {
            var TaskId = Request["TaskId"];
            var CourseId = Request["CourseId"];//课程id
            var ChapterId = Request["ChapterId"];//章id
            var SectionId = Request["SectionId"];//节id
            string wheres = " and  ForeignkeyId=" + TaskId + " and [Types]=1 ";
            var checkcount = commonbll.GetRecordCount("bsi_ExercisePracticeChapters", wheres);
            string sql = "";
            if (checkcount > 0)
            {
                sql = "update bsi_ExercisePracticeChapters set CourseId = '" + CourseId + "',ChapterId = '" + ChapterId + "',SectionId = '" + SectionId + "' where Types = 1 and ForeignkeyId = " + TaskId;
            }
            else {
                sql = "insert into bsi_ExercisePracticeChapters values('" + CourseId + "','" + ChapterId + "','" + SectionId + "','" + TaskId + "','1','" + UserId + "',GETDATE())";
            }
