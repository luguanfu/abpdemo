    ...
    public class NoticeboardController : BaseController
    {
        ...
        public int AddBulletincontent()
        {
            try
            {
                ...
                string list = "NoticeTitle,NoticeContent,ReleaseTime,NoticeState,AddUserId,AddTime,Types";//列
                string vlaue = "@NoticeTitle,@NoticeContent,@ReleaseTime,@NoticeState,@AddUserId,@AddTime,@Types";
                //新增公告表
                SqlParameter[] pars = new SqlParameter[]
                                            {
                                                ...
                                                new SqlParameter("@Types",UserType),//区分（1.管理员新增，2.教师）
                                                new SqlParameter("@NoticeState",2),//新增默认为草稿
                                            };
        ...
        public int UpdateBulletinState()
        {
            ...
            int NoticeState = Convert.ToInt32(Request["NoticeState"]);
            var resultcount = commonbll.UpdateInfo("bsi_Notice", "NoticeState="+ NoticeState + ",ReleaseTime='" + DateTime.Now + "'", " and ID in(" + Noticeid + ")");
            if (resultcount > 0)
            {
                return 1;
            }
        ...
        {
            DataTable dt = commonbll.GetListDatatable(" a.ID,NoticeTitle,NoticeContent,ClassId", " bsi_Notice as A left join dbo.bsi_NoticeClass as B on a.ID=b.NoticeID", " and a.ID=" + Request["id"]);
            return JsonConvert.SerializeObject(dt);
        ...
        public int Edit()
        {
            string Bulletintitle = Request["Bulletintitle"];
            string Bulletincontent = Request["Bulletincontent"];
            var ID = Request["ID"];
            var ClassID = Request["AddTeamId"];


            var resultcount = commonbll.UpdateInfo("bsi_Notice", " NoticeTitle='" + Bulletintitle + "',NoticeContent='" + Bulletincontent + "',AddTime='" + DateTime.Now + "'", " and ID in(" + ID + ")");
             commonbll.DeleteInfo("bsi_NoticeClass", " and NoticeID=" + ID);
            if (resultcount > 0)
            {
                ...
                string list2 = "NoticeID, ClassId,AddUserId,AddTime";//列
                string vlaue2 = "@NoticeID, @ClassId, @AddUserId,@AddTime";
                var AddTeam = ClassID.Split(',');
                for (int i = 0; i < AddTeam.Length; i++)
                {
                    var TeamId = AddTeam[i];//岗位编号
                    SqlParameter[] pars2 = new SqlParameter[]
                                            {
                                                new SqlParameter("@NoticeID",ID),
                                                new SqlParameter("@ClassId",TeamId),
                                                new SqlParameter("@AddUserId",UserId),
                                                new SqlParameter("@AddTime",DateTime.Now)
                                            };
