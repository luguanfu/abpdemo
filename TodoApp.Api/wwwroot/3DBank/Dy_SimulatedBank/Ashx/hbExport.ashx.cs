...
namespace Dy_SimulatedBank.Ashx
{
    ...
    public class hbExport : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Session["UserType"] != null)
            {
                UId = context.Session["UserId"].ToString();//用户ID
                UserType = context.Session["UserType"].ToString();//1管理员2教师3学生4裁判
                UserNo = context.Session["UserNo"].ToString();//登录账号
                TeacherSchoolId = context.Session["USchool"].ToString();//教师属于的院校
...
#pragma warning restore CS0618 // “Workbook.Open(string)”已过时:“Use Workbook(string) constructor method instead.”
                    ...
                    string QB_Custom1 = string.Empty;//试题标识
                    ...
                    try
                    {
                        ...
                        for (int i = 1; i < wk.Worksheets[0].Cells.Rows.Count; i++)
                        {
                            ...
                            if (wk.Worksheets[0].Cells[i, 0].Value != null)
                            {
                                ...
                                {
                                    QB_Custom1 = wk.Worksheets[0].Cells[i, 9].Value.ToString().Trim();
                                }


                                // QB_Custom1 试题表示 是否存在
                                DataTable dt = commonbll.GetListDatatable("ID", "tb_HB_QuestionBQ", " and AddUserId=" + UId + " and QuestonBQName='" + QB_Custom1 + "'");
                                if (dt.Rows.Count > 0)
                                {
                                    QB_Custom1 = dt.Rows[0]["ID"].ToString();//标识ID
                                }
                                else
                                {
                                    string bqtable = "tb_HB_QuestionBQ"; //表名
                                    string bqlist = "QuestonBQName, BQ_Kind, AddTime, AddUserId";//列
                                    string bqvlaue = "@QuestonBQName, @BQ_Kind, @AddTime, @AddUserId";
                                    SqlParameter[] pars = new SqlParameter[] 
                                    { 
                                        new SqlParameter("@QuestonBQName",QB_Custom1),
                                        new SqlParameter("@BQ_Kind",UserType),
                                        new SqlParameter("@AddUserId",UId),
                                        new SqlParameter("@AddTime",DateTime.Now)
                                    };
                                    var resultcount = commonbll.AddIdentity(bqtable, bqlist, bqvlaue, pars);
                                    QB_Custom1 = resultcount.ToString();//自增ID
...
#pragma warning restore CS0618 // “Workbook.Open(string)”已过时:“Use Workbook(string) constructor method instead.”
                            ...
                            try
                            {
                                ...
                                for (int i = 1; i < wk.Worksheets[0].Cells.Rows.Count; i++)
                                {
                                    ...
                                    if (wk.Worksheets[0].Cells[i, 1].Value != null && wk.Worksheets[0].Cells[i, 2].Value != null)
                                    {
                                        ...
                                        if (wk.Worksheets[0].Cells[i, 11].Value != null)
                                        {
                                            QB_Custom1 = wk.Worksheets[0].Cells[i, 11].Value.ToString().Trim();
                                        }
                                        ...
                                        else
                                        {
                                            ...
                                            var qbresultcount = commonbll.AddIdentity(bqtable, bqlist, bqvlaue, pars);
                                            QB_Custom1 = qbresultcount.ToString();//自增ID
                ...
                if (action == "Analysis_Import")
                {
                    try
                    {
                        var allId = context.Request["allId"];
                        HttpPostedFile hpFile = context.Request.Files[0];
                        //获得浏览器端 传过来 第一个文件选择框的数据  
                        string filePath = "/Export";
                        //获得上传上来的文件名称  
                        ...
                        string fileNameNew = DateTime.Now.Year + "" + DateTime.Now.Month + "" + DateTime.Now.Day + "" + DateTime.Now.Hour + "" + DateTime.Now.Minute + "" + DateTime.Now.Second + "" + DateTime.Now.Millisecond + "环节" + allId + "解析" + fileName.Substring(fileName.LastIndexOf('.'));
                        //获得 要保存的物理路径  
                        ...
                        //修改数据库数据
                        SqlHelper.ExecuteNonQuery("update bsi_ProcessAnalysis set wjurl='/Export/" + fileNameNew + "' where Id="+ allId);
