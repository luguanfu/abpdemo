...
namespace Dy_SimulatedBank.Controllers
{
    public class CommonTableController : BaseController
    {
        ...
        public string LoadTableData()
        {
            ...
            where = "";
            if (table == "tb_daily_plkhmxcx" || table == "tb_daily_dfdkmxcx")
            {
                where += " and plan_id=35 ";
            }
            if (table == "tb_daily_rcjycx")
            {
                where += " and plan_id=46 ";
            }

            if (table == "tb_daily_voucher")
            {
            ...
            if (table == "tb_daily_djzflscx")
            {
                where += " and plan_id=2 ";
            }
            if (table == "tb_daily_sfcl")
            {
                where += " and plan_id=2 ";
            }
            ...
            return JsonConvert.SerializeObject(JsonResultPagedListsNew(page_count, page.PageIndex, page.PageSize, dt2));
