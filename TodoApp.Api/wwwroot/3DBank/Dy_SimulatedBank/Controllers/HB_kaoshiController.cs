...
{
    public class HB_kaoshiController : BaseController
    {
        //
        // GET: /HB_kaoshi/考试
...
(select ClassName from tb_Class  where C_ID=" + ClassId + ") as ClassName";//列
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        var resultcount = commonbll.GetRecordCount("tb_ExaminationResult", " and ER_EId=" + Eid + " and ER_PId='" + Pid + "' and ER_MId=" + UserId + " and ER_Type=1 and ER_State=0");
                        if (resultcount > 0)
                        {
                            km.Isallow = "1";
                        }
                    ...
                    else
                    {
                        ...
                        DataTable timeDt = commonbll.GetListDatatable("CD_Time", "tb_CountDown", " and CD_Custom3=1   and CD_EId=" + Eid + " and CD_PId='" + Pid + "' and CD_MId=" + UserId);
                        ...
                        if (dt.Rows[0]["E_Type"].ToString() == "2")//练习模式
                        {
                            //时间记载表上加时长
                        ...
                        {
                            //考试模式  直接以结束时间为倒计时
                            km.TestStartDateTime = Convert.ToDateTime(dt.Rows[0]["E_EndTime"]).ToString("yyyy-MM-dd HH:mm:ss");
                        }
        ...
        public string UpDateTime()
        {
            ...
            var resulutcount = commonbll.GetRecordCount("tb_CountDown", "  and CD_EId=" + Eid + " and CD_PId='" + Pid + "' and CD_MId=" + UserId + " and CD_Custom1=1 and CD_Custom3=" + Type);
            if (resulutcount == 0)
            {
 ...
 e.ED_QBId=q.QuestionBId", " and QB_Type=" + QB_Type + " and  ED_EId='" + Eid + "' and  ED_PId='" + Pid + "' and ED_MId='" + UserId + "' and ED_Type=1 and ED_State=0 and len(ED_Content)>0 and ED_Content!='' ");
            return JsonConvert.SerializeObject(dt);
        }
 ...
 e.ED_QBId=q.QuestionBId", " and ED_EId='" + Eid + "' and  ED_PId='" + Pid + "' and ED_MId='" + UserId + "' and ED_QBId=" + QBId + " and ED_Type=1 and ED_State=0");
            return JsonConvert.SerializeObject(dt);
        }
        ...
        public string AddExaminationDetails()
        {
            ...
            if (dctijiao == "1")
            {
                var checkcount = commonbll.GetRecordCount("tb_ExaminationResult", " and ER_EId=" + Eid + " and ER_PId='" + Pid + "' and ER_MId=" + UserId + " and ER_Type=1 and ER_State=0");
                if (checkcount > 0)
                {
                    return "88";
                }
            ...
            var checkcount2 = commonbll.GetRecordCount("tb_ExaminationDetails", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + " and ED_QBId='" + QBId + "' and ED_Type=1 and ED_State=0 ");
            if (checkcount2 > 0)
            {
                //修改
                string sql = @"update tb_ExaminationDetails set ED_Content='" + ED_Content + "',ED_OkNo='" + ED_OkNo + "',ED_Goal='"
                    + ED_Goal + "' where ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + " and ED_QBId='" + QBId + "' and ED_Type=1 and ED_State=0";
...
@ED_OkNo, @ED_Goal, @ED_State,@ED_Operator, @ED_AddTime, @ED_Custom2)";
                SqlParameter[] pars = new SqlParameter[] 
                {
                    ...
                    new SqlParameter("@ED_MId",UserId),
                     ...
                     new SqlParameter("@ED_Operator",UserId),
        ...
        public string AddExaminationResult()
        {
            ...
            var resulutcount = commonbll.GetRecordCount("tb_CountDown", " and CD_EId=" + Eid + " and CD_PId='" + Pid + "' and CD_MId=" + UserId + " and CD_Custom1=1 and CD_Custom3=1");
            if (resulutcount == 0)
            {
                //如果在提交后在重复刷新 
                return "1";
 ...
 e.EP_QBId=q.QuestionBId", " and EP_PId='" + Pid + "' order by QB_Type");
                ...
                if (allsqldt.Rows.Count > 0)
                {
                    ...
                    {
                        //查询是否做过
                        var resultcount = commonbll.GetRecordCount("tb_ExaminationDetails", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + " and ED_QBId='" + allsqldt.Rows[i]["QuestionBId"] + "' and ED_Type=1 and ED_State=0 ");
                        if (resultcount == 0)
                        {
                            //没做过新增明细记录
                            txtsql += @"insert into  tb_ExaminationDetails(ED_EId, ED_PId, ED_QBId, ED_MId,ED_Type, ED_Content, 
ED_OkNo, ED_Goal, ED_State,ED_Operator, ED_AddTime, ED_Custom2)
values('" + Eid + "', '" + Pid + "','" + allsqldt.Rows[i]["QuestionBId"] + "','" + UserId + "','1', '','错误','0', '0','" + UserId + "','" + DateTime.Now + "','" + UserNo + "'); ";
                ...
                //1.
                commonbll.DeleteInfo("tb_ExaminationDetails", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + "  and ED_Type=1 and ED_State=1  ");
                //2.
                SqlParameter[] parsto = new SqlParameter[] { new SqlParameter("@ED_State", "1") };
                commonbll.UpdateInfo("tb_ExaminationDetails", "ED_State=@ED_State", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + "  and ED_Type=1 and ED_State=0", parsto);
                //3.
                var SumSocres = commonbll.GetListSclar("SUM(ED_Goal)", "tb_ExaminationDetails", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + "  and ED_Type=1 and ED_State=1  ");
                //4.
                SqlParameter[] parsex = new SqlParameter[] { new SqlParameter("@ER_State", "1") };
                commonbll.UpdateInfo("tb_ExaminationResult", "ER_State=@ER_State", " and ER_EId=" + Eid + " and ER_PId='" + Pid + "' and ER_MId=" + UserId + " and ER_Type=1 and ER_State=0 ", parsex);
                ...
                SqlParameter[] parsreult = new SqlParameter[] 
                {
                    ...
                    new SqlParameter("@ER_MId",UserId),
                    ...
                    new SqlParameter("@ER_Operator",UserId),
                ...
                //6.
                commonbll.DeleteInfo("tb_CountDown", " and CD_Custom3=1 and CD_EId='" + Eid + "' and CD_PId='" + Pid + "' and CD_MId='" + UserId + "'");
                return "1";
            }
            catch
            {
        ...
        public string DelExamResult()
        {
            ...
            commonbll.DeleteInfo("tb_ExaminationDetails", " and ED_EId=" + Eid + " and ED_PId='" + Pid + "' and ED_MId=" + UserId + "  and ED_Type=1 and ED_State=0  ");
            return "1";
        }
        ...
        public string GetExaminationResultBys()
        {
            string Pid = Request["Pid"];
            string Eid = Request["Eid"];
            DataTable dt = commonbll.GetListDatatable("ER_Score", @"tb_ExaminationResult", "and ER_EId=" + Eid + " and ER_PId='" + Pid + "' and ER_MId=" + UserId + " and ER_Type=1 and ER_State=0 ");
            return JsonConvert.SerializeObject(dt);
        }
