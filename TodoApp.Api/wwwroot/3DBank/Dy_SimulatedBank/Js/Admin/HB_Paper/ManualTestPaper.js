...
function bindCollegeList() {
    $.ajax({
        url: '/Admin/HB_Paper/Gettb_HB_QuestionBQ',
        Type: "post",
        dataType: "json",
        async: false,
        data: {},
...
function DQcheckeNum(Id) {
    ...
    if (dqnum.length > 0) {
        ...
        if (parseInt(STNUM) != 0) {
            ...
            //共计 赋值
            $("#ZGSTS" + Id).html(parseFloat(Calculation).toFixed(2));
        }
    ...
    //试卷总分赋值
    $("#PScore").html(parseFloat(ZGScores).toFixed(2));
...
function bindIngfo(page) {
    ...
    var SelQuestonBQName = $("#SelQuestonBQName").val();
    var PageSize = 10;
    ...
    $.ajax({
        ...
        data: { "MPId": MPId, "SelQuestonBQName": SelQuestonBQName, "PaperQB_Type": PaperQB_Type, "PaperQB_Description": PaperQB_Description, "PaperStateK": PaperStateK, "PaperIsUse": PaperIsUse, "page": page, "PageSize": PageSize },
        success: function (tb) {
            var html = '';
