...
function GetListById() {
    $.ajax({
        Type: "post",
        dataType: "json",
        url: '/Admin/HB_Practice/GetListById?EId=' + getQueryString('EId'),
        async: false,
...
function BindDll() {
    $.ajax({
        url: '/Admin/HB_Practice/GetListTeam',
        type: 'POST',
        dataType: 'json',
        async: false,
        success: function (data) {
        ...
        success: function (data) {
            if (data == "1") {
                ...
                layer.msg('操作成功', { icon: 1, time: 800 }, function () {
                    window.location.href = '/Admin/HB_Practice/Index';
                });
