    ...
    $.ajax({
        ...
        success: function (tb) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '&nbsp;';
                html += ' <a href="javascript:void(0);" onclick="Choice(' + data[i]["EId"]+')" class=" btn-info btn-sm"><i class="fa fa-lightbulb-o m-r-xxs"></i>关联章节</a>';
                html += '</td>';
...
function See(EId, E_Kind) {
    ...
    $.ajax({
        url: '/Admin/HB_Practice/GetListById?EId=' + EId,
        type: 'POST',
        dataType: 'json',
        async: false,
        success: function (data) {
            if (data.length > 0) {
                ...
                if (E_Kind == "2") {
                    ...
                    $.ajax({
                        url: '/Admin/HB_Practice/GetListTeamByInId?Id=' + teamlist,
                        type: 'POST',
                        dataType: 'json',
                        async: false,
                        success: function (d) {
    ...
    $.ajax({
        ...
        success: function (tb) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<td><a onclick="ChoiceChapters(' + data[i]["ID"] + "," + data[i]["checkcurriculumid"] + "," + data[i]["checkchapterid"] + ')"class=" btn-primary btn-sm  m-r-sm">关联此章节</a></td>';
                html += '</tr>';
            }
...
function Edit(EId, E_IsState) {
    ...
    window.location.href = '/Admin/HB_Practice/EditIndex?EId=' + EId;
}
    ...
    layer.confirm('您确定要激活所选考试吗？', {
       ...
       function () {
           $.ajax({
               type: "POST",
               dataType: "text",
               url: '/Admin/HB_Practice/EditIsState',
    ...
    layer.confirm(deletitle, {
        ...
        function () {
            $.ajax({
                type: "POST",
                dataType: "text",
                url: '/Admin/HB_Practice/DelExamination',
...
//习题关联章节
function ChoiceChapters(sectionId, culumid, chapterid) {
    var TaskId = $("#ReEId").val();
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Admin/HB_Practice/IinkExercisePracticeChapters',
        data: { "sectionId": sectionId, "CourseId": culumid, "chapterid": chapterid, "TaskId": TaskId },
        success: function (data) {
            if (data == "1") {
                layer.closeAll();//关闭所有弹出框
                bindIngfo();
