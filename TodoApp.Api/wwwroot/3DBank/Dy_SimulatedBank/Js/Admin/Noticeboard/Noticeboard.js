...
function BindDll() {
    $.ajax({
        type: "POST",
        dataType: "json",
        url: '/Admin/Noticeboard/GetTeacherList',
        async: false,
...
function GetNoticeList(page) {
    var PageSize = 10;
    $.ajax({
        url: '/Admin/Noticeboard/GetNoticeList',
        Type: "post",
        ...
        success: function (tb) {
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i]["NoticeState"] == 1) {
                        html += '<td> <a onclick="isbfb(' + data[i]["ID"] + ',' + data[i]["NoticeState"] + ')" class=" btn-primary btn-sm  m-r-sm"><i class="fa fa-pencil m-r-xxs"></i>取消发布 </td>'
                    }
                    if (data[i]["NoticeState"] == 2) {
                        html += '<td> <a onclick="isfb(' + data[i]["ID"] + ',' + data[i]["NoticeState"] + ')" class=" btn-primary btn-sm  m-r-sm"><i class="fa fa-pencil m-r-xxs"></i>发布'
                        html += '<a onclick="update(' + data[i]["ID"] + ')" class=" btn-primary btn-sm  m-r-sm"><i class="fa fa-pencil m-r-xxs"></i>编辑 </a> </td>';
                    }
...
function AddExamination() {
    ...
    if (edit == 1) {
        $.ajax({
            type: "POST",
            dataType: "text",
            url: '/Admin/Noticeboard/AddBulletincontent',
        ...
        $.ajax({
            type: "POST",
            dataType: "text",
            url: '/Admin/Noticeboard/Edit?ID=' + ID,
            data: { "Bulletintitle": Bulletintitle, "Bulletincontent": Bulletincontent, "AddTeamId": AddTeamId + "" },
            success: function (data) {
                if (data == 1) {
                    layer.closeAll();//关闭所有弹出框
...
//发布
function isfb(Noticeid, NoticeState) {
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Admin/Noticeboard/UpdateBulletinState',
        data: { "Noticeid": Noticeid, "NoticeState": 1 },
        success: function (data) {
            if (data == 1) {
                layer.closeAll();//关闭所有弹出框
                layer.msg('操作成功', { icon: 1 });
...
//取消发布
function isbfb(Noticeid, NoticeState) {
    $.ajax({
        type: "POST",
        dataType: "text",
        url: '/Admin/Noticeboard/UpdateBulletinState',
        data: { "Noticeid": Noticeid, "NoticeState": 2},
        success: function (data) {
            if (data == 1) {
                layer.closeAll();//关闭所有弹出框
                layer.msg('操作成功', { icon: 1 });
...
function setEdit(id){
    $.ajax({
        type: "POST",
        dataType: "json",
        url: '/Admin/Noticeboard/GetListById?id=' + id,
        async: false,
        success: function (data) {
            if (data.length > 0) {
               
                $("#Bulletintitle").val(data[0]["NoticeTitle"]);
                $("#Bulletincontent").val(data[0]["NoticeContent"]);

                var teamidtxt =[];
                for (var i = 0; i < data.length;i++) {
                    teamidtxt.push(data[i]["ClassId"]);
                }
               
                $("#AddTeamId").val(teamidtxt);
            }
