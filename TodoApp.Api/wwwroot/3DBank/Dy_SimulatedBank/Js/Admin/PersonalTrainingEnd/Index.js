...
function bindIngfo(page) { 
    var PageSize = 10; 
    $.ajax({
        url: '/Admin/PersonalTrainingEnd/GetList',
        Type: "post",
        ...
        success: function (tb) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<td>';
                html += '<a  href="javascript:void(0);"onclick="seturl(' + data[i]["ID"] + ',' + data[i]["PracticeState"] +')" class=" btn-primary btn-sm  m-r-sm"><i class="fa fa-pencil m-r-xxs"></i>查看成绩 </a>';
...
function seturl(ID) {
    ...
    window.location.href = '/Admin/PersonalTrainingEnd/PracticeStudentInfo?PracticeId=' + ID + '&AllType=' + getQueryString("AllType");
}
