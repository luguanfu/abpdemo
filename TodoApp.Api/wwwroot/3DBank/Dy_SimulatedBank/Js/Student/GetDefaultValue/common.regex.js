...
(function (win, doc, _) {
    ...
    function bindExecSelect(element) {
        ...
        $(element).parent().find("input").blur(function (e) { //失去光标 
            $(element).click();
        });
        $(element).parent().find("select").change(function (e) { //选择
            $(element).click();
        });
        $(element).on("click", function () { //快查按钮点击事件
            $(this).attr("vl-exec", "true");
            $(this).attr("src", "/Js/Student/GetDefaultValue/img/banksousoured.png");
            ...
            var defaultData = $("#DefaultValue").val();
            ...
            if (defaultData != "") {
                ...
                for (var h = 0; h < dataArray1.length; h++) {
                    ...
                    if (dataArray1[h].substring(0, 6) == formid) {
                        ...
                        if (keydata.indexOf('sle_') > -1) {
                            elementId = $(element).next().prev().attr("id");
                        }
                        if (keydata == elementId && elementId != undefined) {
                            havedata = '1';
                            defaultData = dataArray1[h];
                            break;
                        } else {
                ...
                var curConId = $(element).prev().attr("id");
                if (curConName.indexOf('sle_') > -1) {
                    curConId = $(element).next().prev().attr("id");
                }

                if (curConName != curConId && curConId != undefined) {
                ...
                if (targeArray.length == 3) {
                    ...
                    if (curConName.indexOf("sle_") > -1) {
                        ...
                        istxt = $(element).next().prev().attr("id");
                        //alert("下拉框的id是:" + istxt);
                    }
                    else {
