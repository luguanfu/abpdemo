/***************************************************************
  FileName:考试 考试明细 javascript
  Copyright（c）2018-金融教育在线技术开发部
  Author:袁学
  Create Date:2018-8-01
 ******************************************************************/
...
$(function () {
    ...
    var IsallowId = $("#IsallowId").val();

    if (parseInt(IsallowId) == 0) {//考试结束时间未到不查看
        layer.msg('考试时间未结束，不能查看成绩明细！', { icon: 2, time: 800 }, function () {
            window.close();
        });
        return;
    }
        ...
        success: function (data) {
            if (data != null && data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i]["Goal"] != null && data[i]["Goal"] != "null") {
                    ...
                    if (data[i]["Goal"] == null || data[i]["Goal"] == "null") {
                        html += '<td class="text_blue">0</td>';
                    } else {
                        html += '<td class="text_blue">' + data[i]["Goal"] + '</td>';
                    }


                    if (data[i]["Goal"] == null || data[i]["Goal"] == "null") {
                        html += '<td class="text_red">0.00%</td>';
                    } else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data != null && data.length > 0) {
                ...
                //当前题多少分
                $("#newQBId").html(parseFloat(data[0]["EP_Score"]).toFixed(2));
