...
(function (win, doc, _) {
        ...
        $(element).on("click", function () {//快查按钮点击事件
            ...
            if(defaultData != "")
            {
              ...
              //010102#sle_qxlx#1-有有效期;txt_zjdqr#20240215;sle_hcjg#00-公民身份证号码与姓名一致&且存在照片;sle_hsjg#1-真实;sle_bz#CNY-人民币;sle_zqfs#1-密码;sle_cxmm#N-否;sle_sflm#Y-是;sle_tctdbz#1-通存通兑;#txt_zjhm#080101:txt_zjhm
                ...
                //根据业务数据,进行查询是否存在的.:
                var targeArray =  dataArray[dataArray.length-1].substring(1).split('#');
                
                // 当前控件名:
                var curConName = targeArray[0];
                
                var targetFormdata = targeArray[1].split(';');
                console.log("bb1b:"+targeArray + ":" + targeArray[1] + ":" + dataArray[dataArray.length-1]);
                // 如果是要查询已有业务,查询对了,就显示默认值,错了,就提示输入值不存在
                if (targeArray.length ==2)
                {
                    //其它业务
                     console.log("bb1b11:");
                     $.ajax({
                        ...
                        data: { data: targeArray[0] + ";" +targeArray[1]+";" + $(element).prev().val() + ";" + examid + ";" + userid  + ";findtable"},
                ...
                else
                {
                    ...
                    // 数据
                    if (targeArray[2] != $(element).prev().val() )
                    {
                    //alert("核心系统提示:["+ $(element).prev().val()+"]帐号不存在");
                     layer.alert("核心系统提示:["+ $(element).prev().val()+"]帐号不存在");
                        return; 
                 ...
                 console.log("bb:"+ formidtmp + ":" +formid );
                if (formidtmp==formid && curConName==$(element).prev().attr("id"))
                {
                    ...
                    else
                    {
                       ...
                       // alert("核心系统提示:["+curkeyConValue+"]帐号不存在");
                        //layer.alert("核心系统提示:["+curkeyConValue+"]帐号不存在");
                    }
                }
            }
            else
            ...
            success: function (row) {
                ...
                else
                {
                    ...
                    if (bank.isCheck) {
                            ...
                            // 取出序列号
                             $.ajax({
                                ...
                                msguser= msguser+data.data;
                                 updateForm(bank.data + columnName + data.data,tdata );
                        ...
                        {
                           updateForm(bank.data,tdata,msguser); 
                        }
    ...
    function updateForm(data,tdata,msguser) {
