...
var current_taskid;
 $(function () {  
     ...
     js_element.setAttribute("src", "/busi/js/ed.common.js");
   ...
   isCheckVoucherNo(formid,function(cdata){ 
      ...
      //生成假提交按钮
       if(yw_data.sysname=="核心系统"){   
         fake_btnsubmit_html="<button type=\"button\" style=\"margin-right: 0px;\" id=\"fake_btnsubmit\">提交（F6）</button>"; 
       }else{
 ...
 function printWin() { 
     ...
     layer.open({
         ...
         // content: '/User/indexone?formid=010001'+"&tellerId="+tellerId+"&examid="+examid+"&banksiteid="+banksiteid+"&DepartmentId"+DepartmentId+"&planid="+planid
             ...
             //if (formid == "080704") {
             //    //现金轧账提交成功并且打印窗口关闭后触发  
             //    var url = "/user/indexone?formid=080705&tellerId=" + tellerId + "&examid=" + examid + "&banksiteid=" + banksiteid + "&DepartmentId=" + DepartmentId + "&planid=" + planid;
             //    location.href = url;
             //}
             //if (formid == "080705") {
             //    //凭证轧账提交成功并且打印窗口关闭后触发 
